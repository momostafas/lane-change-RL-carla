...
class World(object):
    ...
    def setLaneChange (self, client, value):
        tm = client.get_trafficmanager(8000)
        tm.force_lane_change(self.player,value)
    def setWaypoints (self, waypoints):
        # Find next waypoint 2 meters ahead.
        waypoint = random.choice(waypoints[0].next(0.5))
...
class LaneInvasionSensor(object):
    def __init__(self, parent_actor, hud):
        ...
        # reference.
        weak_self = weakref.ref(self)
        self.sensor.listen(lambda event: LaneInvasionSensor._on_invasion(weak_self, event, parent_actor))

    @staticmethod
    def _on_invasion(weak_self, event, player):
        self = weak_self()
        if not self:
            return
...
def game_loop(args):
    ...
    try:
        ...
        # world.setplayerPhysics
        # print('way', waypoint_list[0])
        # world.setWaypoints(waypoint_list)
        ...
        while True:
            ...
            # print('lane', laneSTR)                
            if (laneSTR == 'Right'):
                world.setLaneChange(client, True)
            elif (laneSTR == 'Left'):
                world.setLaneChange(client, False)
            elif (laneSTR == 'Both'):
                world.setLaneChange(client, False)
